<template>
  <div class="admin-page">
    <div class="admin-header">
      ...
      <h1>Menus</h1>
      <button class="primary-button" @click="openCreate">+ Nuevo menu</button>
    </div>
    <template v-if="loading"><div class="list-state"><div class="state-spinner" /> Cargando...</div></template>
    <template v-else-if="error"><div class="error-state"><p>{{ error }}</p><button class="secondary-button" @click="loadMenus">Reintentar</button></div></template>
    <template v-else-if="menus.length === 0"><div class="empty-state"><p>No hay menus registrados</p><button class="primary-button" @click="openCreate">+ Crear primer menu</button></div></template>
    ...
    <ModalWrapper v-if="showModal" @close="closeModal">
      <div class="modal-form">
        ...
        <div class="field"><label>Modulo</label><select v-model="form.moduleId" required><option value="">Seleccionar modulo...</option><option v-for="mod in allModules" :key="mod.id" :value="mod.id">{{ mod.name }}</option></select></div>
        ...
        <div class="modal-actions">
          ...
          <button class="primary-button" :disabled="saving" @click="saveMenu">{{ saving ? 'Guardando...' : 'Guardar' }}</button>
        </div>
      </div>
    </ModalWrapper>
  </div>
...
async function saveMenu() {
  ...
  try {
    ...
    if (editingMenu.value) await menuService.update(editingMenu.value.id, dto)
    else await menuService.create(dto as { name: string; moduleId: string })
    closeModal(); await loadMenus()
...
async function handleDelete(m: Menu) {
  ...
  try { await menuService.remove(m.id); await loadMenus() }
