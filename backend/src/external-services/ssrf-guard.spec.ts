...
describe('assertSafeProbeTarget', () => {
  ...
  it('rechaza protocolos distintos de http y https', async () => {
    await expect(
      assertSafeProbeTarget('file:///etc/passwd'),
    ).rejects.toBeInstanceOf(BadRequestException);
    await expect(
      assertSafeProbeTarget('gopher://interno:70/'),
    ).rejects.toBeInstanceOf(BadRequestException);
  });
  ...
  it('rechaza loopback y rangos privados', async () => {
    ...
    for (const target of blocked) {
      await expect(assertSafeProbeTarget(target)).rejects.toBeInstanceOf(
        BadRequestException,
      );
    }
  });
  ...
  it('rechaza IPv4 mapeada en IPv6 que apunta a loopback', async () => {
    ...
    await expect(
      assertSafeProbeTarget('http://[::ffff:127.0.0.1]/health'),
    ).rejects.toBeInstanceOf(BadRequestException);
  });
  ...
  it('permite rangos privados cuando ALLOW_PRIVATE_PROBE_TARGETS esta activo', async () => {
    ...
    await expect(
      assertSafeProbeTarget('http://127.0.0.1:3006/health'),
  ...
  it('acepta una direccion publica', async () => {
    await expect(
      assertSafeProbeTarget('https://93.184.216.34/health'),
    ).resolves.toMatchObject({
