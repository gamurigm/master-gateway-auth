...
| **Backend** | `.` | `npm ci && npm run prisma:generate -w backend && npm run build:backend` | `node backend/dist/src/main.js` | N/A | `DATABASE_URL`, `FRONTEND_ORIGIN`, `JWT_ISSUER`, `JWT_AUDIENCE`, `JWT_SECRET`, `JWT_EXPIRES_IN`, `TEMP_JWT_SECRET`, `TEMP_JWT_EXPIRES_IN`, `REFRESH_JWT_SECRET`, `REFRESH_JWT_EXPIRES_IN`, `INTERNAL_API_KEY`, `INTERNAL_ALLOWED_SERVICES`, `SEED_ADMIN_EMAIL`, `SEED_ADMIN_PASSWORD`, `NODE_ENV=production` |
