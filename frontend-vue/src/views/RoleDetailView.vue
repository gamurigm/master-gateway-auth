    ...
    <div class="admin-card">
      <template v-if="activeTab === 'users'">
        ...
        <table class="crud-table">
          <thead><tr><th>Email</th><th>Nombres</th><th>Estado</th><th /></tr></thead><tbody>
            <tr
              v-for="a in role.users"
              :key="a.id"
            >
              <td>{{ a.user.email }}</td><td>{{ a.user.firstName }}</td><td>
                <span
                  class="badge"
                  :class="a.user.estado === 'ACTIVO' ? 'badge-active' : 'badge-inactive'"
                >{{ a.user.estado }}</span>
              </td><td>
                <button
                  class="icon-btn delete"
                  @click="unassign('user', a.user.id)"
                >
                  <AppIcon
                    name="trash-2"
                    size="16"
      ...
      <template v-if="activeTab === 'modules'">
        ...
        <table class="crud-table">
          <thead><tr><th>Codigo</th><th>Nombre</th><th>Estado</th><th /></tr></thead><tbody>
            <tr
              v-for="a in role.modules"
              :key="a.id"
            >
              <td><code>{{ a.module.code }}</code></td><td>{{ a.module.name }}</td><td>
                <span
                  class="badge"
                  :class="a.module.estado === 'ACTIVO' ? 'badge-active' : 'badge-inactive'"
                >{{ a.module.estado }}</span>
              </td><td>
                <button
                  class="icon-btn delete"
                  @click="unassign('module', a.module.id)"
                >
                  <AppIcon
                    name="trash-2"
                    size="16"
      ...
      <template v-if="activeTab === 'menus'">
        ...
        <table class="crud-table">
          <thead><tr><th>Nombre</th><th>URL</th><th>Estado</th><th /></tr></thead><tbody>
            <tr
              v-for="a in role.menus"
              :key="a.id"
            >
              <td>{{ a.menu.name }}</td><td><code>{{ a.menu.url }}</code></td><td>
                <span
                  class="badge"
                  :class="a.menu.estado === 'ACTIVO' ? 'badge-active' : 'badge-inactive'"
                >{{ a.menu.estado }}</span>
              </td><td>
                <button
                  class="icon-btn delete"
                  @click="unassign('menu', a.menu.id)"
                >
                  <AppIcon
                    name="trash-2"
                    size="16"
