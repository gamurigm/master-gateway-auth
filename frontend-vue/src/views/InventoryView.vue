    ...
    <div class="inventory-config">
      <div class="field api-field">
        ...
        <div style="display:flex; gap:8px">
          <input v-model="apiUrl" placeholder="/inventario" />
          <button class="primary-button" :disabled="loadingProducts" @click="queryProducts">Consultar</button>
        </div>
      </div>
    </div>
    <template v-if="loadingProducts">
      <div class="list-state"><div class="state-spinner" /> Consultando inventario...</div>
    </template>
    <template v-else-if="queryError">
      <div class="error-state">
        <AppIcon name="alert-triangle" size="24" />
        <p>{{ queryError }}</p>
        <button class="secondary-button" @click="queryProducts">Reintentar</button>
      </div>
    </template>
    ...
    <table v-else class="crud-table">
      ...
      <tbody>
        <tr v-for="p in products" :key="p.sku">
          ...
          <td>{{ p.name }}</td>
          <td>{{ p.stock }}</td>
          <td><span class="badge" :class="p.status === 'ACTIVO' ? 'badge-active' : 'badge-inactive'">{{ p.status }}</span></td>
        </tr>
      </tbody>
    </table>
  </div>
